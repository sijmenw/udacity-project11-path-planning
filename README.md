...
# Code description
The code can be roughly split into four sections:
 1. determine if action is necessary
 2. generating trajectories
 3. calculating cost for each trajectory
 4. selecting trajectory
...
A switch will not be considered if doing so would result in driving off road.

For each decision, three trajectories are generated, one at 100% speed, one at 85% speed and one at 70% speed.
